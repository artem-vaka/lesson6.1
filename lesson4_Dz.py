...
players = [
    {"name": "Воїн",   "hp": 120, "max_hp": 120, "attack": 15, "defense": 10, "gold": 0, "inventory": ["меч", "щит"]},
    {"name": "Маг",    "hp": 80,  "max_hp": 80,  "attack": 25, "defense": 4,  "gold": 0, "inventory": ["посох", "книга заклинань"]},
    {"name": "Лучник", "hp": 100, "max_hp": 100, "attack": 18, "defense": 7,  "gold": 0, "inventory": ["лук", "стріли (20)"]},
    {"name": "Злодій", "hp": 90,  "max_hp": 90,  "attack": 20, "defense": 5,  "gold": 0, "inventory": ["кинджал", "відмички"]},
]
...
def explore_location(player):
    ...
    location_index = choose_location()
    show_header(f"ЛОКАЦІЯ {location_index + 1}")
...
def main():
    ...
    for i in range(5):
        if not explore_location(player):
            show_slow("\nГру завершено.")
            break
        show_slow(f"\nПісля випробування ти маєш {player['hp']} HP і {player['gold']} золота.")

    else:
        show_slow("\nВітаємо! Ти пройшов всі випробування.")
